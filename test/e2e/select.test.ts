...
describe('select e2e ', () => {
  ...
  it('should select all', async () => {
    ...
    `);
    expect(db.q).toMatchInlineSnapshot(`"SELECT * FROM comment"`);
  ...
  it('should select all with specific columns', async () => {
    ...
    expect(db.q).toMatchInlineSnapshot(
      `"SELECT id, author_id, content_md FROM comment"`,
    );
    expect(db.args).toMatchInlineSnapshot(`Array []`);
  });
  ...
  it('should allow selecting by a set of possible values', async () => {
    ...
    const users1 = await selectAnyOf(db, {
      id: new Set([
        ...
        '00000000-1f62-4f80-ad29-3ad48a03a47f',
      ]),
    });
    expect(db.q).toMatchInlineSnapshot(
  ...
  it('should combine singular and plural where clauses', async () => {
    ...
    expect(db.q).toMatchInlineSnapshot(
      `"SELECT id, author_id, metadata FROM comment WHERE author_id = $1 AND doc_id = ANY($2)"`,
    );
    expect(db.args).toMatchInlineSnapshot(`
      Array [
        "d0e23a20-1f62-4f80-ad29-3ad48a03a47f",
  ...
  it('should allow multiple plural where clauses', async () => {
    ...
    expect(db.q).toMatchInlineSnapshot(
      `"SELECT * FROM comment WHERE author_id = ANY($1) AND doc_id = ANY($2)"`,
    );
    expect(db.args).toMatchInlineSnapshot(`
      Array [
        Array [],
  ...
  describe('joins', () => {
    it('should join to another table with all columns', async () => {
      const selectJoin = docTable.select({join: {author: 'created_by'}});
      const docs = await selectJoin(db);
      expect(db.q).toMatchInlineSnapshot(
        `"SELECT t1.*, to_jsonb(t2.*) as author FROM doc as t1 JOIN users AS t2 ON t1.created_by = t2.id"`,
      );
      expect(db.args).toMatchInlineSnapshot(`Array []`);
    ...
    it('should join to another table with select columns', async () => {
      ...
      expect(db.q).toMatchInlineSnapshot(
        `"SELECT t1.id, t1.metadata, to_jsonb(t2.*) as author FROM comment as t1 JOIN users AS t2 ON t1.author_id = t2.id"`,
      );
      expect(db.args).toMatchInlineSnapshot(`Array []`);
      expect(comments).toMatchInlineSnapshot(`
        Array [
    ...
    it('should join to multiple tables', async () => {
      ...
      expect(db.q).toMatchInlineSnapshot(
        `"SELECT t1.*, to_jsonb(t2.*) as author, to_jsonb(t3.*) as doc FROM comment as t1 JOIN users AS t2 ON t1.author_id = t2.id JOIN doc AS t3 ON t1.doc_id = t3.id"`,
      );
      expect(db.args).toMatchInlineSnapshot(`Array []`);
      expect(comments).toMatchInlineSnapshot(`
        Array [
    ...
    it('should join with selectByPrimaryKey', async () => {
      const select = docTable.selectByPrimaryKey({
      ...
      expect(db.q).toMatchInlineSnapshot(
        `"SELECT t1.*, to_jsonb(t2.*) as author FROM doc as t1 JOIN users AS t2 ON t1.created_by = t2.id WHERE t1.id = $1"`,
      );
      expect(db.args).toMatchInlineSnapshot(`
        Array [
          "01234b31-1f62-4f80-ad29-3ad48a03a36e",
    ...
    it('should join with selectByPrimaryKey and specific columns', async () => {
      ...
      expect(db.q).toMatchInlineSnapshot(
        `"SELECT t1.id, t1.metadata, to_jsonb(t2.*) as author FROM comment as t1 JOIN users AS t2 ON t1.author_id = t2.id WHERE t1.id = $1"`,
      );
      expect(db.args).toMatchInlineSnapshot(`
        Array [
          "01234567-1f62-4f80-ad29-3ad48a03a36e",
