...
function validate(
  ...
  function diagnosticShouldBeIgnored(diagnostic: ts.Diagnostic) {
    ...
    return /'.+' is declared but (never used|its value is never read)./.test(
      messageText,
    );
  }
...
function parseAssertions(sourceFile: ts.SourceFile): Assertions {
  ...
  while (true) {
    ...
    const match =
      /^( *)(?:(\$Expect(?:TypeSnapshot|Type|Error))|(\^\?))(?: (.*))?$/.exec(
        commentMatch[1],
      ) as
        | [
            never,
            string,
            '$ExpectTypeSnapshot' | '$ExpectType' | '$ExpectError' | undefined,
            '^?' | undefined,
            string?,
          ]
        | null;
    ...
    switch (directive) {
      ...
      case '^?': {
        ...
        const expectedPrefix =
          text.slice(lineStarts[line - 1], commentCol + 2 + whitespace.length) +
          '   ';
